  ...
  return (
    <div className="bg-white min-h-screen mx-auto p-4 bg-gradient-to-r from-yellow-50 from-20% via-purple-50 via-50% to-green-50">
      <div className='flex flex-col items-center w-full '>
        <div className="bg-white p-4 rounded-xl mb-4 flex flex-col gap-2 items-center w-2/4">
          ...
          <div className="flex items-center  text-black text-sm font-bold px-4 py-3" role="alert">
            <p>{loadingMsg}</p>
          </div>
          <Image className="h-2/6 w-auto rounded-full border-black border-solid" src={demoNFT} alt="DemoNFT" />
          
          <p className="text-md mb-2 text-center">{`Utilizing zero-knowledge proofs in session-based access control mechanisms can provide a robust solution for achieving a harmonious blend of user security and a frictionless user experience.`}</p>
          <p className="text-md mb-2 text-center">{`This application serves as an example demonstrating a zk session-based smart contract account, eliminating the need for third-party involvement. `}</p>
        ...
        {transactionReceipt != '' 
          && <p className="text-gray-600 mb-1">UserOperation Receipt: 
              <a href={transactionReceipt} className="text-blue-600 font-semibold"
              target="_blank"
...
const CircularLoading = () => {
  return (
    <svg aria-hidden="true" className="w-5 h-5 mr-2 text-white animate-spin dark:text-gray-600 fill-violet-500" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
